import { Typography, Card, CardContent, CardMedia, Dialog, DialogActions, DialogContent, DialogTitle, CircularProgress, Box, IconButton, Divider } from "@mui/material";
import Grid2 from "@mui/material/Grid2";
import { useState, useEffect } from "react";
import { Stack, Button, Select, MenuItem, FormControl, InputLabel, DialogContentText } from "@mui/material";
    ...
    return (
        <>
            <Box sx={{ minHeight: "68.6vh" }}>
                <Typography variant="h4" align="center" sx={{ mt: 2, mb: 3, color: "#c98c26", fontWeight: "bold" }}>Listar Obras de un Artista</Typography>
                    ...
                    <Grid2 item size={{ xs: 12, sm: 8, md: 6 }}>
                        <Stack component="form" onSubmit={handleSubmit} spacing={4} sx={{ p: 4, borderRadius: 2, boxShadow: 3, backgroundColor: "#f9f9f9" }}>
                            <FormControl fullWidth>
                                ...
                                <Select id="idartista" value={datos.idartista} onChange={handleChangeSel} label="Artista">
                                    {artistas.map((artista) => (
                                        <MenuItem key={artista.idartista} value={artista.idartista}>{artista.nombre} ({artista.paisDeNacimiento})</MenuItem>
                                    ))}
                                </Select>
                            </FormControl>
                            <Button variant="contained" type="submit" fullWidth sx={{ bgcolor: "#c98c26", "&:hover": { bgcolor: "#a76f1f" }, fontWeight: "bold" }}>Listar Obras del Artista</Button>
                        </Stack>
                    </Grid2>
                </Grid2>
                    ...
                    {obras.map((obra) => (
                        <Grid2 item xs={12} sm={6} md={4} key={obra.idobra}>
                            <Card sx={{ borderRadius: 2, boxShadow: 3 }}>
                                <CardMedia component="img" alt={obra.nombre} height="200" image={obra.imagen_url} sx={{ objectFit: "cover" }} />
                                <CardContent>
                                    <Typography variant="h6" fontWeight="bold">{obra.nombre}</Typography>
                                    <Typography variant="body2" color="text.secondary">{obra.descripcion}</Typography>
                                    <Divider sx={{ my: 1 }} />
                                    <Typography variant="body1" fontWeight="bold" color="primary">{`Precio: $${obra.precio}`}</Typography>
